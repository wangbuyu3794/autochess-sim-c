...
`V2.6` 已提供 raylib 图形版实验入口。默认不构建图形版，避免影响终端版。

当前图形版支持查看棋盘、备战席和商店，并能用鼠标完成基础准备阶段操作：点击商店购买，点击备战席选中，点击己方棋盘部署或移动，点击空备战席撤回，点击左侧装备库存给选中单位穿戴或替换装备，按钮支持刷新、锁店、自动部署/装备和进行一场战斗。战斗后右侧面板会显示最近一场战斗摘要，包括回合、胜负、双方生命变化、上场人数和装备奖励。`V2.5` 起会优先加载 Windows 中文字体，使界面文案、按钮和提示更中文友好。
