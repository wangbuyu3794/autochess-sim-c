...
static void gui_init_state(GuiState *state)
{
    ...
    gui_set_message(state, "V2.13：展示打磨完成，提示更紧凑。");
}
...
static void gui_draw_info_panel(const GameContext *game, const GuiState *state)
{
    ...
    DrawText("V2.13 展示打磨", 42, 122, 20, (Color){40, 48, 62, 255});
...
static void gui_draw_message_bar(const GameContext *game, const GuiState *state)
{
    ...
    DrawText(gui_next_action_hint(game, state), 780, 697, 13, LIGHTGRAY);
}
...
int main(void)
{
    ...
    InitWindow(GUI_SCREEN_WIDTH, GUI_SCREEN_HEIGHT, "AutoChess-C Showcase Polish");
    gui_init_resources();
    SetTargetFPS(60);
